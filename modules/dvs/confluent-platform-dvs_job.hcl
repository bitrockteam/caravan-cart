job "confluent-platform-dvs" {
  ...
  group "confluent-platform" {
    ...
    task "broker" {
      ...
      env {
        KAFKA_BROKER_ID = "1"
        KAFKA_ZOOKEEPER_CONNECT = "$${NOMAD_HOST_IP_http_zook}:2181"
        KAFKA_LISTENER_SECURITY_PROTOCOL_MAP = "PLAINTEXT:PLAINTEXT,PLAINTEXT_HOST:PLAINTEXT"
        KAFKA_ADVERTISED_LISTENERS = "PLAINTEXT://kafka-dvs.service.consul:$${NOMAD_HOST_PORT_http_broker},PLAINTEXT_HOST://localhost:29092"
        ...
        CONFLUENT_METRICS_REPORTER_ZOOKEEPER_CONNECT = "$${NOMAD_HOST_IP_http_zook}:2181"
