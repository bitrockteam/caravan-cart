...
for t in $TOPICS
do
IS_TOPIC=$(kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --list | grep -c $t)
if [ $IS_TOPIC -eq 0 ]; then
  kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --create --topic $t --replication-factor 1 --partitions 1
fi
done
...
IS_TOPIC=$(kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --list | grep -c $t)
if [ $IS_TOPIC -eq 0 ]; then
  kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --create --topic $t --replication-factor 1 --partitions 1  --config retention.ms=86400000
fi
done
...
IS_TOPIC=$(kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --list | grep -c $t)
if [ $IS_TOPIC -eq 0 ]; then
  kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --create --topic $t --replication-factor 1 --partitions 15
fi
done
...
IS_TOPIC=$(kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --list | grep -c $t)
if [ $IS_TOPIC -eq 0 ]; then
  kafka-topics --bootstrap-server ${KAFKA_BOOTSTRAP_SERVERS} --create --topic $t --replication-factor 1 --partitions 1 --config cleanup.policy=compact,delete
fi
done
